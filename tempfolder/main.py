    ...
    try:
        with pdfplumber.open(file_path) as pdf:
            for page_idx, page in enumerate(pdf.pages, start=1):
                ...
                if not text or len(text) < MIN_CHUNK_LENGTH_CHARS:
                    continue
                cleaned_text = clean_text(text)
                if MIN_CHUNK_LENGTH_CHARS < len(cleaned_text) < MAX_CHUNK_LENGTH_CHARS:
                    chunks.append({
                        "document": filename,
...
def find_top_candidates(intent_embedding: np.ndarray, chunks: List[Dict[str, Any]], model: SentenceTransformer) -> List[Dict[str, Any]]:
    ...
    chunk_embeddings = model.encode(chunk_texts, batch_size=EMBEDDING_BATCH_SIZE, show_progress_bar=False, device='cpu')
    ...
    for candidate in chunks[:DUPLICATE_CHECK_POOL]:
        text_signature = candidate["refined_text"][:150].lower()
...
def main():
    ...
    intent_embedding = model.encode([intent_query], device='cpu').reshape(1, -1)
